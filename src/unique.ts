...
/**
 * Given filter arguments, return true if the provided value is
 * unique to the array, false otherwise.
 */
function unique<T>(value: T, index: number, array: ArrayLike<T>): boolean

/**
 * Given an {@link ArrayLike}, return an {@link ArrayLike} containing only unique values.
 * @param arrayLike
 */
function unique<T extends string | ArrayLike<unknown>>(arrayLike: T): T

/**
 * Bound to an {@link ArrayLike}, return an {@link ArrayLike} containing only unique values.
 * @param this
 */
...
function unique(
    ...
    if (this) return unique(this as ArrayLike<unknown>)

    // handle filter signature
    if (arrayContainingItem) {
        const item = arrayLikeOrItem
        return (
            findIndex.call(arrayContainingItem, value =>

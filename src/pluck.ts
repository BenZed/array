import { nil, TypeGuard } from '@benzed/types'
...
function pluck(this: unknown, ...args: unknown[]): unknown[] {
    const [input, predicate, maxToRemove] = (
        args.length <= 1 ? [this, ...args] : args
    ...
    if (reverse) count = -count

    for (
        let i = reverse ? input.length - 1 : 0;
        results.length < count && (reverse ? i >= 0 : i < input.length);
        i += reverse ? -1 : 1
    ) {
        const value = input[i]
        if (!predicate(value, i, input)) continue
    ...
    for (const index of indexes) spliceInput(index, 1)
